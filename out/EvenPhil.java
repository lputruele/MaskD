public class EvenPhil implements Runnable { 
  ...
  public EvenPhil(Bool forkL,Bool forkR) {
  ...
  synchronized private boolean action2(){
    if (s == state.hungry && forkR.getValue() && !hasL && !hasR){
  ...
  synchronized private boolean action3(){
    if (s == state.hungry && !forkR.getValue() && !hasR && !hasL){
      s=state.hungry;
      return true;
    }
    else{
  ...
  synchronized private boolean action4(){
    if (s == state.hungry && hasR && forkL.getValue() && !hasL){
